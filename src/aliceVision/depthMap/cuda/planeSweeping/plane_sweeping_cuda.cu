                         ...
                         int volDimX, int volDimY, int volDimZ, bool verbose)
{
    ...
    for (int z=0;z<volDimZ;z++)
    {
            volume_compute_DP1_kernel<<<gridvol, blockvol>>>(
                    xySlice_dmp.getBuffer(), xySlice_dmp.getPitch(),
                    ovol_dmp.getBuffer(), ovol_dmp.getBytesPaddedUpToDim(1), ovol_dmp.getBytesPaddedUpToDim(0),
                    ivol_dmp.getBuffer(), ivol_dmp.getBytesPaddedUpToDim(1), ivol_dmp.getBytesPaddedUpToDim(0),
                    volDimX, volDimY, volDimZ, z);
            cudaThreadSynchronize();
    };
    */
